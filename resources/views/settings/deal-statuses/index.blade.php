<x-layout>
    <x-slot name="heading">{{ __('messages.deal_statuses') }}</x-slot>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-5">
                ...
                <form method="POST" action="{{ route('settings.deal-statuses.store') }}" class="space-y-3">
                    @csrf
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" value="{{ old('slug') }}" required placeholder="e.g. in_progress"
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition {{ $status->trashed() ? 'opacity-50' : '' }}"
                            ...
                            <td class="px-4 py-2">
                                ...
                                <form x-show="editing" method="POST"
                                    action="{{ route('settings.deal-statuses.update', $status) }}"
                            ...
                            <td class="px-4 py-2">
                                <div class="flex items-center justify-end gap-1">
                                    @if(! $status->trashed())
                                        ...
                                        <form method="POST" action="{{ route('settings.deal-statuses.destroy', $status) }}">
                                            @csrf @method('DELETE')
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('settings.deal-statuses.restore', $status->id) }}">
                                            @csrf
                                            <button type="submit"
